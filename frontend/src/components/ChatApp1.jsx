...
const ChatApp1 = () => {
  ...
  const hasUnreadMessages = (friendId) =>
    unreadFriendIds.includes(String(friendId));

  // Initialize socket connection
  fetch(`${API_URL}/api/unread-friends`, {
    ...
    .then((data) => setUnreadFriendIds(data.unreadFriendIds || []))
    .catch((err) => console.error("Error fetching unread friends:", err));
  ...
  const sendFriendRequest = async () => {
    try {
      const response = await fetch(`${API_URL}/api/send-friend-request`, {
        method: "POST",
        headers: {
  ...
  const acceptFriendRequest = async (requestId) => {
    try {
      const response = await fetch(`${API_URL}/api/accept-friend-request`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
